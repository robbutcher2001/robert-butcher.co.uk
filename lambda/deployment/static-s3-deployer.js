...
exports.handler = (event, context, callback) => {
  ...
  var downloadedZip = fs.createWriteStream(workingSubfolder + '.zip');
...
var downloadZip = (s3Client, sourceZipLocation, downloadedZip) => {
...
var extractDistFiles = (workingSubfolder, downloadedZip) => {
  return new Promise((resolve, reject) => {
...
var uploadFiles = (s3Destination, distFiles) => {
  return Promise.all(
    distFiles.map(distFile => {
      ...
      const params = {
        Key: distFile.key,
        Body: distFile.body,
        ContentType: mime.lookup(distFile.key)
      };
...
var signalRomeoNotDone = (job, context, err) => {
  const codePipeline = new AWS.CodePipeline();
  const params = {
    jobId: job.id,
    failureDetails: {
